...
class KospiAgentCalcData(models.Model):
    '''
    - description: KOSPI agent data updated daily to calculate absolute score
    - period: 20080701 ~
...
class KosdaqAgentCalcData(models.Model):
    '''
    - description: KOSDAQ agent data updated daily to calculate absolute score
    - period: 20080701 ~
    - data: (date, code, tp, buy_cumsum, apps, tp_buy_cumsum)
    - agent: {'individual':ind, 'foreign_retail': for,
            'institution': ins, 'etc_corporate': cor,
...
class DefactoReg(models.Model):
    '''
    - description: monthly regression data
    - period: 200807 ~
